...
#**************************************************************************************************************************
    # Subrotuine to read anyfile and return a 2d array
#**************************************************************************************************************************
...
sub sending{
    ...
    # --------------------------
     chomp %coeff; chomp $z;
        if ($z<5){$coeff{517}=$coeff{517}} else {$coeff{517}=-$coeff{517}}
    ...
    my @tt;chomp @tt;
    foreach my $atom(1..$n_atoms){
    ($tt[$atom][0],$tt[$atom][1],$tt[$atom][2],$tt[$atom][3])=($file_array_1[$atom][0],$p[$atom][0],$p[$atom][1],$p[$atom][2]);
        #print  "$tt[$atom][0]---$tt[$atom][1]---$tt[$atom][2]---$tt[$atom][3]\n";
    }
  ...
  # ------------------------------------------
    ...
    my $ssslar = scalar @f_trim;
    
    
    @p=();
    return (\@f_trim,$ssslar);
}# end, sub sending
...
sub reading_files{
    ...
    return (\@final_1,$n_atoms,\%coeff,\@modes,\@file_array_1);
